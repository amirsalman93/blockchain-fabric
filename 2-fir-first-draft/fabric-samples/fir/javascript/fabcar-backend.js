...
async function invokeCommand(inputs, socket) {
    try {
        ...
        if (!userExists) {
            ...
            socket.emit('RESPONSE', {type: 'ERROR', payload: 'user1 doesn\'t exist.' });
            return;
        }
        ...
        if (inputs.fcn == 'createCar')
        {
            await contract.submitTransaction(inputs.fcn, inputs.args[0], inputs.args[1], inputs.args[2], inputs.args[3], inputs.args[4]);
            console.log('Transaction has been submitted');
            //socket.emit('RESPONSE', {type: 'INFO', payload: 'Transaction has been submitted' });
            
        }
        else if (inputs.fcn == 'changeCarOwner')
        {
            await contract.submitTransaction(inputs.fcn, inputs.args[0], inputs.args[1]);
            console.log('Transaction has been submitted');
            socket.emit('RESPONSE', {type: 'FEED', payload: `Request for TRANSFER for ${inputs.args[0]} to ${inputs.args[1]} processsed` });
        }
        ...
        errorString = `Failed to evaluate transaction: ${error}`;
        console.error(errorString);

        socket.emit('RESPONSE', {type: 'ERROR', payload: errorString });
        
        process.exit(1);
    }
}

async function queryCommand(inputs, socket) {
    try {
        // Create a new file system based wallet for managing identities.
        const walletPath = path.join(process.cwd(), 'wallet');
        const wallet = new FileSystemWallet(walletPath);
        ...
        if (inputs.fcn == 'queryAllCars')
        {
            const result = await contract.evaluateTransaction(inputs.fcn);
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);

            socket.emit('RESPONSE', {type: 'INFO', payload: JSON.parse(result) });
        }
        else if (inputs.fcn == 'queryCar')
        {
            const result = await contract.evaluateTransaction(inputs.fcn, inputs.args[0].toString());
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
    ...
    socket.on('REQUEST', (req) => {
        ...
        switch (req.action)
        {
            ...
            case "QUERYALL":
                ...
                queryCommand({fcn : 'queryAllCars', args : '[]'}, socket);
                    break;
            case "TRANSFER":
                socket.emit('RESPONSE', {type: 'START', payload: `Request for TRANSFER for ${req.data.ID} to ${req.data.newOwner} received` });
                invokeCommand({fcn: 'changeCarOwner', args: [req.data.ID.toString() , req.data.newOwner.toString()]}, socket);
                
                break;
            case "CREATE":
                socket.emit('RESPONSE', {type: 'START', payload: `Request for CREATE for ${req.data.ID} received` });
                invokeCommand({fcn: 'createCar', args: [req.data.ID.toString(), req.data.make.toString(), req.data.model.toString(), req.data.color.toString(), req.data.owner.toString()]}, socket);
